...
| Output `normal_map` | `(3, H, W)` | Unit normals in camera space; borders / invalid → 0 |
...
In tensor layout `points[v, u, :]`:

- \(\mathbf{t}_x\) = `points[:, 2:, :] - points[:, :-2, :]`
- \(\mathbf{t}_y\) = `points[2:, :, :] - points[:-2, :, :]`
...
- **Border pixels** (one pixel wide): normal set to \(\mathbf{0}\) (no neighbors for central differences).
- **Invalid depth:** non-finite or \(d \leq \epsilon\) → normal \(\mathbf{0}\).
...
- Raw array: `normals.npy`, shape `(3, H, W)`
- RGB preview: \(\text{RGB} = 0.5\,(\hat{\mathbf{n}} + 1)\) clamped to \([0,1]\)
